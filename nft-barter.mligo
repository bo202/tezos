...
sudo docker run --rm -v "$PWD":"$PWD" -w "$PWD" ligolang/ligo:0.19.0 compile-contract nft-barter-map.mligo main --output-file=nft-barter-map.tz
...
[{id = 1; amount=a_1;}, {id=2; amount=a_2}, ...]
*)
...
*)
let main (x, storage : exchange * owner_map) : operation list * owner_map =
    
    (*  adj_token_id0 and adj_token_id1 has the amounts exchanged 
        for each token_id.  The amounts are used to adjust the storage.
    *)
    let counterparty0 : counterparty      = x.0             in
    let owner0        : owner_address     = counterparty0.0 in
    let token_addr0   : token_address     = counterparty0.1 in
    let adj_token_id0 : token_id_map      = counterparty0.2 in

    let counterparty1 : counterparty      = x.1             in
